...
if mdata-get postfix_mysqluser 1>/dev/null 2>&1 && mdata-get postfix_mysqlpassword 1>/dev/null 2>&1 && mdata-get postfix_mysqldbname 1>/dev/null 2>&1; then
  ...
  chmod 0700 /opt/qutic/bin/generate_local_domains_maps
  
  CRON='0 * * * * /opt/qutic/bin/generate_local_domains_maps'
  ...
  chmod 0700 /opt/qutic/bin/generate_dkim_domains_maps

  CRON='5 * * * * /opt/qutic/bin/generate_dkim_domains_maps'
  (crontab -l 2>/dev/null || true; echo "$CRON" ) | sort | uniq | crontab
fi